...
def get_business_analyst_agent():
    ...
    agent_config = config['business_model_analyst']
    
    # Import tool lazily to avoid circular imports or early failure
    from src.tools.pdf_tools import PDFCatalogTool, PDFContentReaderTool
    
    return Agent(
        role=agent_config['role'],
        goal=agent_config['goal'],
        backstory=agent_config['backstory'],
        tools=[PDFCatalogTool(), PDFContentReaderTool()],
        llm=get_qwen_llm(),
        verbose=True,
        allow_delegation=False
    )
